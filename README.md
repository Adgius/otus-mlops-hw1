[Домашнее задание 1](#домашнее-задание-1)

[Домашнее задание 2](#домашнее-задание-2)

[Домашнее задание 3](#домашнее-задание-3)

[Домашнее задание 4](#домашнее-задание-4)

[Домашнее задание 5](#домашнее-задание-5)

[Домашнее задание 6](#домашнее-задание-6)

[Домашнее задание 7](#домашнее-задание-7)


# Домашнее задание 1

Цель проекта - разработать за полгода конкурентоспособную антифрод-систему.  Основная часть системы должна размещаться на внешних ресурсах. Система должна выдерживать высокие нагрузки. Также необходимо обеспечить конфиденциальность данных. За первые три месяца предоставить MVP.

Так как было определено, что у конкурентов максимум 2% мошеннических транзакций приводят к потере средств, то будем использовать метрику *False Negative Rate*:
$$FNR = \frac{FN}{FN+TP} * 100\% $$, где *FN* - число мошеннических транзакций, приведших к потере средств; *TP* - число распознаных мошеннических транзакций моделью. Данная метрика должна быть меньше 2%.

Для контроля недовольства клиентов – *False Positive Rate*. Доля ложных блокировок транзакций честного пользователя от общего числа транзакций.


$$FPR = \frac{FP}{FP+TN} * 100\%$$, где *FP* - число ложных срабатываний; *TN* - число обычных транзакций, которые распознаны как **не**мошеннические. Данная метрика должна быть меньше 5%.

Бизнес-метрика -- ущерб от мошеннических транзакций за месяц. Не должна превышать 500 тыс. рублей.

Особенности предстоящего проекта:

![image](https://github.com/Adgius/otus-mlops-hw1/assets/78685114/98e9fb05-7232-4f7a-a554-68f40fd44e61)

Основные фунциональные задачи:
1. Подготовка и сбор данных
2. Создание хранилища для данных
3. Разработка модели
4. Тесты
5. Деплой модели
6. Поддержка

# Домашнее задание 2

Основные команды:

**Создание бакета**

`yc storage bucket create --name otus-mlops-hw2 --default-storage-class cold --max-size 161061273600 --public-read --public-list`

Ссылка на s3: 

*s3://otus-mlops-hw2/fraud-data/*

**Копирование файлов с бакета на бакет**

`s3cmd cp -r --acl-public s3://mlops-data/fraud-data s3://otus-mlops-hw2/`

**Перемещение на hdfs**

`hadoop distcp s3a://otus-mlops-hw2/fraud-data/ /user/ubuntu/input/`

![image](https://github.com/Adgius/otus-mlops-hw1/assets/78685114/dfda7f39-5964-4100-b3ba-fb485d93ab6c)

Для поддержания работоспособности текущего сервиса (s3 + hdfs) необходимо ~26500 рублей
- S3 (150ГБ, холодное): ~250 руб/мес
- HDFS (мастер нода с публичным ip + дата нода 3 хоста) ~26000 руб/мес


В качестве оптимизации можно уменьшить число хостов в датаноде и использовать более дешевые конфигурации железа.

# Домашнее задание 3

**Создание бакета под очищенные данные**

`yc storage bucket create --name otus-mlops-data-clear --default-storage-class cold --max-size 161061273600 --public-read --public-list`

**Подключаемся к мастер ноде и запускаем jupyter notebook**

Чтобы в дальнейшем к нему подключиться, необходимо при создании кластера разрешить подключение на 8888 порт.

`jupyter notebook --no-browse --port 8888 --ip=*`

**Устанавливаем библиотеку для автоматического поиска зависимостей для Spark**

`pip install findspark`

**Исследуем данные**
[Ссылка на ноутбук](https://github.com/Adgius/otus-mlops-hw1/blob/master/3/data-exploration.ipynb)

**Составляем финальный скрипт**
[Ссылка на скрипт](https://github.com/Adgius/otus-mlops-hw1/blob/master/3/clean-data.py)

**Запускаем финальный скрипт для очистки**

В качестве аргументов передаем ключи от сервисного аккаунта s3.
`python clean-data.py [Идентификатор ключа] [Cекретный ключ]`

# Домашнее задание 4

### Периодический запуск процедуры очистки датасета мошеннических финансовых транзакций

**Для начала создаем ВМ, где будет развернут Ariflow. Рекомендуется минимум 8 ГБ оперативной памяти** 

**Подключаемся к ВМ по ssh**

**Клонируем этот репозиторий на ВМ**

`git clone https://github.com/Adgius/otus-mlops-hw1.git`

**Перемещаемся в папку с текущим дз**

`cd otus-mlops-hw1/4/`

**Устанавливаем docker на ВМ, с помощью подготовленного скрипта**

`bash install-docker.sh`

Чтобы изменения вступили в силу перезаходим в терминал

**Для подключения к Spark кластеру будет необходим ssh-ключ, поэтому создаем его**

`ssh-keygen`

**Чтобы ключ работал в Airflow, перемещаем его в рабочий проект**

`cp -R $HOME/.ssh $HOME/otus-mlops-hw1/4/`

**В директории с дз необходимо создать файл с окружением, в который поместим идентификатор пользователя**

`echo -e "AIRFLOW_UID=$(id -u)" > $HOME/otus-mlops-hw1/4/.env`

**Запустим инициализацию базы данных и создание пользователя**

Команды надо выполнять в директории `$HOME/otus-mlops-hw1/4`

`docker compose up airflow-init`

**Запустим Airflow**

`docker compose up -d`

Подключиться к интерфейсу можно через ip ВМ и порта 8080. Пароль и логин: `airflow`

При входе мы увидим такую картину:
![image](https://github.com/Adgius/otus-mlops-hw1/assets/78685114/f1fc824f-0647-45dd-b045-a548d9121940)

Один даг не добавился по причине отсутствия адреса Spark кластера, потому что он еще не создан. Подключение по ssh автоматически появится после успешного завершения первого дага

Для работы дагов требуется указать 4 переменные:
- `ssh-key` - это ssh ключ, который мы создали. Его можно узнать командой `cat $HOME/.ssh/id_rsa.pub`
- `yc_token` - OAuth токен. Его можно узнать на [этой странице](https://cloud.yandex.ru/docs/cli/quickstart)
- `aws_access_key_id` - публичная часть API-ключа от сервисного аккаунта с правами доступа к S3 хранилищу
- `aws_secret_access_key` - секретная часть API-ключа от сервисного аккаунта с правами доступа к S3 хранилищу

Запускаем первый даг и ждем, пока создастся кластер. Взаимодествие происходит через REST-API Яндекса, потому что утилиту `yc` не удалось поставить внутри контейнера с Airflow. Скрипт для создания кластера через `yc` можно найти по пути `otus-mlops-hw1/4/.data/create-cluster.sh`

После успешного завершения первого дага станет доступен второй даг `run_script`

Результат выполнения
![image](https://github.com/Adgius/otus-mlops-hw1/assets/78685114/240f6eb7-7257-4cfa-9529-ba8bd5683a7b)

# Домашнее задание 5

Основные компоненты остались как в дз 4. Что добавилось:
* В файл docker-compose добавил два сервиса: MLflow и базу PostgreSQL
* Добавил скрипт, который добавляет необходимые для запуска переменные окружения

Шаги для запуска (почти всё как в 4 задании):
1. `git clone https://github.com/Adgius/otus-mlops-hw1.git`
2. `ssh-keygen`
3. `cp -R $HOME/.ssh $HOME/otus-mlops-hw1/5/`
4. Добавляем с помощью команды `export` ключи от S3: AWS_ACCESS_KEY_ID и AWS_SECRET_ACCESS_KEY
6. Добавляем переменные окружения командой `bash otus-mlops-hw1/5/create-env.sh`
7. Устанавливаем docker `bash otus-mlops-hw1/5/install-docker.sh`
8. Перезаходим в терминал
9. `cd otus-mlops-hw1/5 ; docker compose up airflow-init`
10. `docker compose up -d`

Airflow доступен на 8080 порту, MLflow на 5000.

Заходим в Airflow, добавляем `yc_token` как в предыдущем дз и запускаем даг *create-spark-cluster*. Ждем окончания создания кластера.

Затем запускаем *run_script*.

По окончании процесса в логах mlflow мы увидим успешный ран:
![image](https://github.com/Adgius/otus-mlops-hw1/assets/78685114/a1d1d1ca-62bc-4293-bd4f-2e8344fa14ca)

Метрики сохранены в базе данных mlflow, а модель сохранена в S3 хранилище:
![image](https://github.com/Adgius/otus-mlops-hw1/assets/78685114/10916806-ff3f-4ce8-b79c-3e8c15396c7e)

Что хочется отметить:
1. В качестве пайплайна был взят случайный лес и минимальный набор дополнительных фичей (описаны в классе *FeatureGenerator* внутри *run_pipeline.py*). Из-за того что некоторые фичи зависели от предыдущего времени, образовывались пропуски, которые заполнялись простым средним, что в свою очередь является не лучшим вариантом. Как можно улучшить:
* использовать больше данных, сейчас для ускорения процесса брался небольшой кусочек данных (примерно 10%) от каждого дня
* заменить модель, добавить больше фичей и выявить из них полезные
* более точечно подойти к проблеме холодного старта и пропусков
2. Оказалось, что на успех прогноза очень влияет фича по наличию случаев фрода в определенном терминале. Получается, наличие фрода в прошлом положительно коррелирует с наличием фрода по данному терминалу в будущем.
3. По дефолту SSHOperator оператор логинится без исполнения файлов .bashrc и .profile в домашнем каталоге. Из-за этого  `spark-submit` работает некорректно. Для решения этой проблемы необходимо добавлять параметр `--login` к `bash`.
4. Чтобы запустить mlflow пришлось упаковывать все библиотеки в виртуальное окружение и отправлять на кластер. В дополнение нужно прописать путь к интерпретатору из этого виртуального окружения.
5. Долго не мог понять, почему на датаноде нет ключей под s3, хотя при запуске mlflow я их указывал. Оказывается при обращению к серверу mlflow, он не предоставляет никаких секретов. Только дает возможность логирования в своей базе postgres. Поэтому ключи надо прокидывать через airlfow в локальное окружение при запуске скрипта в кластере.

# Домашнее задание 6

Отличия от предыдущего дз:
- Переделал генерацию признаков. Теперь стало меньше пропущенных значений, т.к сейчас пропущенные значения можно заменить на ноль (например, если по карте не было транзакций на прошлой неделе, то ставим 0). Подробнее можно посмотреть в классе *FeatureGenerator* внутри *run_pipeline.py*.
- Добавил кастомный валидатор метрик, чтобы метрики отвечали заданным в начале проекта (FPR и FNR).
- Добавил валидацию модели:
  * Доверительный интервал для метрик считается бутстрепом и берется между 2.5% и 97.5% процентилем.
  * Для определения лучшей модели в mlflow введена специальная метрика (можно сказать костыль), которая называется *best_model*. Она может быть равна 1 только в одном из множества mlflow runs, в котором метрики достигают лучшего результата на тестовом датасете. Все последующие runs будут сравниваться с run, где best_model равна 1. Как только метрики очередного run побьют лучший, то ему присвоится best_model = 1, а у предыдущего лидера будет 0.

    Пример логов из airflow по валидации модели:

    ![image](https://github.com/Adgius/otus-mlops-hw1/assets/78685114/5d5c292f-500d-487e-8259-76cd239d0433)

- Метрики. На данный момент в эксперименте с 100 000 записями и логистической регрессией в качестве модели удалось достичь таких метрик:
  
  ![image](https://github.com/Adgius/otus-mlops-hw1/assets/78685114/0b98f2c9-6e51-4ddd-949e-2d8e4f0a858c)

  Исходя из метрик, можно заключить, что модель очень точно предсказывает фрод, если его замечает. Но при этом пропускает множество случаев. 

  Я решил посмотреть, как эту проблему решают другие коллеги. Я нашел [репозиторий](https://github.com/a-chasovsky/perc_credit_card_fraud_detection), в котором метрики качества достигают отличных показателей. Однако меня смутил тот факт, что в качестве фичи берется скользящее среднее по числу фрода по банкоматам за последние n дней. По моему мнению здесь имеет место быть "просачивание" целевой переменной в тренировочный датасет. При попытке воспроизвести эксперимент и отвалидировать полученную модель, я увидел, что важность остальный фичей стремится к нулю. 95% информации для предсказания берется из истории фрода по данному терминалу.

  ![image](https://github.com/Adgius/otus-mlops-hw1/assets/78685114/65f2c26f-0e36-4b62-a22d-e76ee229c17c)

  Также я посмотрел еще одно решение из [kaggle](https://www.kaggle.com/code/shriyashjagtap/the-ultimate-fraud-detection-tool-99-98-acc). В данном решении используется метод upsampling, где соотношение между обычными транзакциями и фродом 1:1. Но в реальности распределение между обычными транзакциями и фродом примерно 14:1.

  В репозитории задания есть ноутбук, который находится в папке .data. В нем я попытался реализовать указанные выше методы без использования pyspark. Метрики полученные бутстрепом представлены ниже:
  
  ![image](https://github.com/Adgius/otus-mlops-hw1/assets/78685114/82cde752-c6bf-46d0-9fff-d97f2d1ca7f2)![image](https://github.com/Adgius/otus-mlops-hw1/assets/78685114/2622bb85-0e1d-4ab2-bbc2-8eec8c922426)

  Как видно FPR уже не ноль, однако есть к чему стремиться. Модель по прежнему пропускает подавляющее большинство фрода.


# Домашнее задание 7

Структура проекта - три сервиса в файле docker compose:
* zookeeper - для управления над kafka
* kafka - наш брокер сообщений. Для учебных целей шифрование отключено (`PLAINTEXT`). Для внутренней коммуникации используется порт 29092, для внешнего доступа 9092. Также создаются два тестовых топика с 1 фактором репликации и 1 партицей: *train* и *predict*.
* kafdrop - для удобной навигации. Порт 9000.

Алгоритм запуска:
1. Добавляем в переменные окружения ip ВМ. `export HOST_IP=$(curl ipinfo.io/ip)`
2. `docker compose up -d`
3. Открываем ноутбук *kafka_producer.ipynb*. Я использовал Google Collab. Исполняем все ячейки, генерируем данные.
4. В Spark-кластере запускаем `spark-submit --packages org.apache.spark:spark-sql-kafka-0-10_2.12:3.0.3 kafka-spark-pipeline.py -i <ip VM>`
5. Смотрим результат в kafdrop.

Структура проекта:
* Сначала генерируются данные в топик *train* в ноутбуке формата {"X": Array[float, 3], "y": int}
* При запуске скрипта все данные считываются и сохраняюся на кластере
* Затем на этих данных обучается модель линейной регрессии
* Предсказания заливаются в топик *predict* в формате {"y": float, "prediction": float}

Пример:

![image](https://github.com/Adgius/otus-mlops-hw1/assets/78685114/30692197-dfc6-4edb-b356-8aadc49b596e)

Что можно улучшить:
* Использовать модель на новых данных, сейчас для обучения и для инференса одинаковые данные
* Добавить ключи к каждой паре X и y для дальнейшей манипуляции над данными

