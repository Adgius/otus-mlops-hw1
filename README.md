...
- Метрики. На данный момент в эксперименте с 100 000 записями и логистической регрессией в качестве модели удалось достичь таких метрик:
  ...
  Модель предсказывает редко, но метко :D

  Однако я решил посмотреть, как эту проблему решают другие коллеги. Я нашел [репозиторий](https://github.com/a-chasovsky/perc_credit_card_fraud_detection), в котором метрики качества достигают отличных показателей. Однако меня смутил тот факт, что в качестве фичи берется скользящее среднее по числу фрода по банкоматам за последние n дней. По моему мнению здесь имеет место быть "просачивание" целевой переменной в тренировочный датасет. При попытке воспроизвести эксперимент важность фичей,
- . Пример расчетов из репы (был взят признак на основе целевой переменной + апсэмлинг). свой ноутбук
